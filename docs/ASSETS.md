...
| Exhibition replay (`apps/web/public/replays/exhibition.command-log.json`) | Generated in this repo (Story 12.12) by `apps/web/scripts/build-exhibition-replay.mts`, against Groq's free-tier API | **Authored in this repo**, except the `rawResponse` strings, which are model output — see below | 2026-08-11 |
| Exhibition reasoning sidecar (`apps/web/public/replays/exhibition.reasoning.json`) | Same run as above | **Model output** — 125 KB of verbatim completions from Groq-hosted `openai/gpt-oss-120b` and `openai/gpt-oss-20b` — see below | 2026-08-11 |
...
### Story 12.12: the exhibition replay, and the one artefact here with third-party content

`exhibition.command-log.json` and `exhibition.reasoning.json` are one Match
between two language models — Groq-hosted `openai/gpt-oss-120b` versus
`openai/gpt-oss-20b`, seed 121200, 57 Decision Points — and they are the flagship
replay `startup.ts` opens the page on. Regenerate with:

```
GROQ_API_KEY=… node --experimental-strip-types --no-warnings \
     --import ./packages/cli/bin/register.mjs apps/web/scripts/build-exhibition-replay.mts
```

**This is the first committed artefact in the repository whose content is not
ours.** Every other row in this table is either licensed art or a document this
project generated from its own deterministic simulation. The sidecar is 125 KB of
verbatim completions from two models, and the log's `rawResponse` fields are the
same text: recorded rather than authored, because that is what makes the replay a
replay (INV-2 — the Match is re-simulated from the log, and the model's words are
the *input* that cannot be reproduced). Both models are OpenAI's open-weight
`gpt-oss` releases served on Groq's free tier, and the output is retained here as
the evidence for a published benchmark result. The two documents were read before
they were committed, per this story's `operator_actions`; there is nothing in them
a model said that should not ship.

Regeneration is **not** byte-reproducible, and that is the one row in this file
where that is true and correct. Everything else here regenerates identically from
a seed. Re-running the generator plays a new Match: the seed, the config hash and
the engine are fixed, but the models' responses are not, so the Actions differ and
so does the Final-State Hash. What is verifiable is the artefact itself — it
replays to its own hash, which `apps/web/src/testing/exhibition-artefact.test.ts`
asserts over the committed file.

The generator paces its own transport against each model's published TPM
(`packages/providers/src/free-tier.config.json`), because at ~1,100 tokens a call
an unpaced Match spends the minute's free-tier allowance in eight Decision Points
and the rest of the log becomes rate-limit bodies. No key is read from anywhere
but the environment, no key is written, and `scripts/assert-no-secret-leak.sh`
passes over the tree with the key present.
